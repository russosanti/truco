...
local function play(steps, humanScore, aiScore, manoVal, pieVal)
    local c = Canto(FLOR_CANTO, 'human', 'flor')
    for _, step in ipairs(steps) do
        if step == 'accept' then c:accept()
        elseif step == 'reject' then c:reject()
        else c:raise(step) end
    end
    return florAward(c.outcome, 'human', manoVal or 35, pieVal or 30,
        humanScore or 0, aiScore or 0)
end
...
T('flor_canto', function(t)
    t('the ladder narrows flor -> contraflor -> al resto -> nothing', function(t)
        ...
        c:raise('contraflor')
        t:assert(c.responder == 'human', 'and it goes back to the other side')
    ...
    t('base flor accepted pays 3 to the better flor', function(t)
        ...
        t:assert(side == 'human' and points == 3, 'mano 35 beats pie 30 for 3')
    ...
    t('Contraflor al resto: falta plus both flores, 6 if refused', function(t)
        ...
        t:assert(side == 'human', 'the better flor takes it')
        t:assert(points == faltaEnvidoValue(10, 5, 'human') + 6,
        ...
        t:assert(points == faltaEnvidoValue(22, 8, 'human') + 6, 'same formula in buenas')
        ...
        t:assert(side == 'human' and points == 6,
