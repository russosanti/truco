...
T('canto (engine + envido binding)', function(t)
    t('opening Envido: responder answers, accept pays cumulative to the winner', function(t)
        local c = Canto(ENVIDO_CANTO, 'player', 'envido')  -- human opens; AI responds
        t:assert(c.responder == 'ai', 'AI must respond to the player open')
        ...
        local side, pts = envidoAward(c.outcome, 'player', 33, 32, 0, 0)
        t:assert(side == 'player' and pts == 2, 'player wins envido, +2')
    end)

    t('reject pays the pre-call pot to the caller (floored at 1)', function(t)
        local c = Canto(ENVIDO_CANTO, 'player', 'envido')
        c:reject()
        local side, pts = envidoAward(c.outcome, 'player', 0, 0, 0, 0)  -- reject ignores values
        t:assert(side == 'player' and pts == 1, 'rejected lone Envido: caller +1')
    end)
    ...
    t('raised chain Envido -> Real: pot 5, availableRaises narrows', function(t)
        ...
        t:assert(c.responder == 'player', 'player responds')
    ...
    t('falta is a ceiling: no raises, accept pays faltaEnvidoValue', function(t)
        local c = Canto(ENVIDO_CANTO, 'player', 'falta')
        ...
        local side, pts = envidoAward(c.outcome, 'player', 33, 30, 10, 5)
        t:assert(side == 'player' and pts == 20, 'falta malas: player 10 -> +20')
    end)
    ...
    t('reject a ceiling pays the pre-falta pot', function(t)
        ...
        t:assert(c.responder == 'player', 'after AI raises falta, player must answer')
        c:reject()
        local side, pts = envidoAward(c.outcome, 'player', 0, 0, 0, 0)  -- reject ignores values
