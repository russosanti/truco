...
T('match_rules', function(t)
    t('chicoWinner: nobody until a side reaches 30', function(t)
        ...
        t:assert(chicoWinner(30, 12) == 'human', 'human at exactly 30 wins')
    ...
    t('chicoWinner: overshooting 30 still wins (falta/truco can jump)', function(t)
        t:assert(chicoWinner(34, 20) == 'human', 'human past 30 wins')
    ...
    t('partidaWinner (best_of_3): only at 2 chicos', function(t)
        t:assert(partidaWinner({ human = 0, ai = 0 }, 'best_of_3') == nil, '0-0 undecided')
        t:assert(partidaWinner({ human = 1, ai = 0 }, 'best_of_3') == nil, '1-0 undecided')
        t:assert(partidaWinner({ human = 1, ai = 1 }, 'best_of_3') == nil, '1-1 undecided -> 3rd chico')
        t:assert(partidaWinner({ human = 2, ai = 0 }, 'best_of_3') == 'human', '2-0 human')
        t:assert(partidaWinner({ human = 1, ai = 2 }, 'best_of_3') == 'ai', '2-1 ai (decisive 3rd)')
    end)

    t('partidaWinner (single_chico): the first chico ends it', function(t)
        t:assert(partidaWinner({ human = 0, ai = 0 }, 'single_chico') == nil, '0-0 undecided')
        t:assert(partidaWinner({ human = 1, ai = 0 }, 'single_chico') == 'human', '1-0 human')
        t:assert(partidaWinner({ human = 0, ai = 1 }, 'single_chico') == 'ai', '0-1 ai')
    end)
end)
